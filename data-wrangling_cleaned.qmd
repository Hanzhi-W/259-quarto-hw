...
author: "John Franchak, modified by Hanzhi Wang"
format: html
editor: visual
---
...
library(here)
```
MComment: Make sure to add a space after # to properly format (ex: #STEP 1 does not have a space, so it just looks like a normal line in the HTML output)
...
```{r}
ppt_long <- ppt %>%
  pivot_longer(cols = starts_with("Seen"),
               names_to = "stim",
               values_to = "watched")
ppt_long <- ppt_long %>%
...
ppt_long <- ppt_long %>%
  mutate(stim = factor(stim, levels = stim_labels, labels = stim_labels),
         watched =
           factor(watched, levels = 1:3, labels = c("Yes","No","Not Sure")))
```
...
```{r}
ds %>% 
  ggplot(aes(x = precision)) + geom_histogram() + geom_vline(xintercept = 2.5)
```
